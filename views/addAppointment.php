                    ...
                    if (isset($errorMsg)) {
                        ...
                        <form action="<?= isset($_GET['id']) ? '?id=' . $_GET['id'] : htmlspecialchars($_SERVER['PHP_SELF']) //permet de renseigner ds la meme page tt en encodant les caratéres speciaux ds un soucis de sécurite
                            ...
                            <div class="form-group">
                                ...
                                <select class="form-select" name="name" aria-label="Default select example" required>
                                    ...
                                    <?php foreach ($listPatients as $patient) : ?>
                                        <option value="<?= $patient->id ?>" <?php if (isset($patient) && isset($result)) { ?> <?= $patient->id == $result->id ? "selected" : null ?> <?php } ?>>
                            ...
                            <div class="form-group">
                                ...
                                <input name="date" type="date" value="<?= $result->dateHour ? date("Y-m-d", strtotime($result->dateHour)) : '' ?>" required class="form-control" min="<?= date('Y-m-d') ?>" id="date">
                                <p class="error"><?= $error['date'] ?? '' ?></p>
                            </div>

                            <div class="form-group">
                                <label for="date">Choix de l'heure * :</label>
                                <input name="time" pattern="<?= REGEX_TIME ?>" type="time" value="<?= $result->dateHour ? date("H:i", strtotime($result->dateHour)) : '' ?>" required class="form-control" min="09:00" max="17:30" id="time">
